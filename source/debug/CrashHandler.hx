...
class Crashie extends FlxState {
	override public function create() {
		...
		for (line in stripClub) {
			...
			crash[i].setFormat('assets/fonts/settings.ttf', 22, FlxColor.WHITE, FlxTextAlign.LEFT);
			crash[i].alpha = 0;
	...
	override public function update(elapsed:Float) if (FlxG.keys.justPressed.ANY) {
		if (FlxG.keys.justPressed.ENTER) FlxG.openURL("https://github.com/NAEL2XD/LuApps/issues");
		CrashHandler.errorMessage = "";
		Dummy.exit();
	}
}